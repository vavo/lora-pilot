window.initDashboard = async function () {
  ...
  try {
    ...
    // CPU
    const cpuPct = Math.min(100, Math.round((data.load_avg[0] / (data.cpu_count || 1)) * 100));
    const cpuBar = document.getElementById("cpu-bar");
    const cpuLbl = document.getElementById("cpu-label");
    if (cpuBar) cpuBar.style.width = `${cpuPct}%`;
    if (cpuLbl) cpuLbl.textContent = `${cpuPct}% of ${data.cpu_count || 1} cores (load ${data.load_avg[0].toFixed(2)})`;
    // GPU(s)
